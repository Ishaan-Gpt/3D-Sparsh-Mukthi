...
/**
 * A live whiteboard: a CanvasTexture plane whose content is redrawn whenever
 * the lesson engine reveals a new title/line, in sync with the teacher's voice.
 */
export function Whiteboard({ position = [0, 0.6, -16.6], rotation = [0, 0, 0], width = 13, height = 6.5 }) {
  ...
  useEffect(() => {
    ...
    wrapText(ctx, board.title || "", W / 2, 78, W - 120, 56);
    ...
    let y = 190;
    for (const line of board.lines.slice(-5)) {
